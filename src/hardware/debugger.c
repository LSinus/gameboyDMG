...
static const char* opcode_mnemonic[256] = {
...
static const char* cb_opcode_mnemonic[256] = {
...
static int disassemble_instr(uint8_t data[3], char* buffer, size_t size) {
  ...
  uint8_t num_bytes = opcode_bytes[opcode];
  switch (num_bytes) {
    case 1: {
      const char* mnemonic = opcode_mnemonic[opcode];
      if (!mnemonic) {
        mnemonic = "*INVALID*";
      }
    ...
    case 2:
      if (opcode == 0xcb) {
        strncpy(temp, cb_opcode_mnemonic[data[1]], sizeof(temp) - 1);
      } else {
        snprintf(temp, sizeof(temp), opcode_mnemonic[opcode], data[1]);
      }
      break;
    case 3:
      snprintf(temp, sizeof(temp), opcode_mnemonic[opcode],
               (data[2] << 8) | data[1]);
      break;
